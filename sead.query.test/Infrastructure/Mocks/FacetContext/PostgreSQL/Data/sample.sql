...
create or replace function sample.sample_per_datatype(p_limit integer)
  returns setof public.tbl_physical_samples
  language sql
as
$$
	with combos as (
	...
	cross join lateral (
	  select p2.*
	  from public.tbl_physical_samples p2
	  join public.tbl_analysis_entities ae  using (physical_sample_id)
...
create view sample.tbl_languages as select * from public.tbl_languages;
...
create view sample.tbl_sample_groups as
    ...
    where sample_group_id in (
        ...
        from sample.tbl_physical_samples ps
    );

create view sample.tbl_datasets as
    select ds.*
    from public.tbl_datasets ds
    where dataset_id in (select dataset_id from sample.tbl_analysis_entities);

create view sample.tbl_sites as
    select s.*
    from public.tbl_sites s
    where site_id in (select site_id from sample.tbl_sample_groups);
...
create view sample.tbl_features as
    ...
    where feature_id in (select feature_id from sample.tbl_physical_sample_features);

create view sample.tbl_site_locations as
    select x.*
    from public.tbl_site_locations x
    where site_id in (select site_id from sample.tbl_sites);
...
create view sample.tbl_projects as
    ...
    where project_id in (select project_id from sample.tbl_datasets);
...
create view sample.tbl_dataset_submissions as
    ...
    where dataset_id in (select dataset_id from sample.tbl_datasets);
...
create view sample.tbl_species_associations as
    ...
    where FALSE; -- skipping due to size
...
create view sample.tbl_taxa_tree_genera as
    ...
    where genus_id in (select genus_id from sample.tbl_taxa_tree_master);

create view sample.tbl_taxa_tree_families as
    select x.*
    from public.tbl_taxa_tree_families x
    where family_id in (select family_id from sample.tbl_taxa_tree_genera);

create view sample.tbl_taxa_tree_authors as
    select x.*
    from public.tbl_taxa_tree_authors x
    where x.author_id in (select author_id from sample.tbl_taxa_tree_master);
