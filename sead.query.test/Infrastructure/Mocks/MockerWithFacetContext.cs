...
namespace SQT
{
    ...
    public class MockerWithFacetContext : IDisposable
    {
        ...
        public virtual ISetting CreateSettings() => new SettingFactory().GetSettings();

        public virtual FacetContext CreateFacetContext() => new FacetContextFactory(Settings.Store).GetInstance();

        public virtual RepositoryRegistry CreateRepositoryRegistry() => new(FacetContext);

        public MockerWithFacetContext(FacetContext facetContext)
        {
            __FacetContext =
                facetContext != null ? new Lazy<IFacetContext>(() => facetContext) : new Lazy<IFacetContext>(CreateFacetContext);
        ...
        public virtual Facet FakeFacet(string facetCode, FacetType t, FacetGroup g, bool is_applicable = true) =>
            new()
            {
                FacetCode = facetCode,
        ...
        public virtual Mock<IPickFilterCompiler> MockPickFilterCompiler(string returnCriteria)
        {
            ...
            mock.Setup(x => x.Compile(It.IsAny<Facet>(), It.IsAny<Facet>(), It.IsAny<FacetConfig2>())).Returns(returnCriteria);
            return mock;
        }
        ...
        protected Mock<IPicksFilterCompiler> MockPicksFilterCompiler(IEnumerable<string> returnPicks)
        {
            ...
            mock.Setup(x => x.Compile(It.IsAny<Facet>(), It.IsAny<List<FacetConfig2>>())).Returns(returnPicks);
            return mock;
        }

        public virtual IPicksFilterCompiler MockPicksFilterCompiler(
            Mock<IPickFilterCompiler> discreteMock,
            Mock<IPickFilterCompiler> rangeMock
        )
        {
        ...
        public virtual Mock<IJoinSqlCompiler> MockJoinSqlCompiler(string returnValue)
        {
            ...
            mockCompiler.Setup(x => x.Compile(It.IsAny<TableRelation>(), It.IsAny<FacetTable>(), It.IsAny<bool>())).Returns(returnValue);
            return mockCompiler;
        }
        ...
        public virtual Mock<ITypedQueryProxy> MockTypedQueryProxy(List<CategoryItem> fakeCategoryCountItems)
        {
            ...
            mockQueryProxy
                .Setup(foo => foo.QueryRows<CategoryItem>(It.IsAny<string>(), It.IsAny<Func<IDataReader, CategoryItem>>()))
                .Returns(fakeCategoryCountItems);
        ...
        /// <param name="returnSql"></param>
        /// <returns></returns>
        public virtual Mock<T> MockCategoryCountSqlCompiler<T>(string returnSql)
            where T : class, ICategoryCountSqlCompiler
        {
            var compiler = new Mock<T>();
            compiler.Setup(c => c.Compile(It.IsAny<QuerySetup>(), It.IsAny<Facet>(), It.IsAny<CompilePayload>())).Returns(returnSql);
            return compiler;
        }
        ...
        public virtual Mock<IIndex<EFacetType, ICategoryCountHelper>> MockCategoryCountHelpers()
        {
            ...
            mockHelpers.Setup(x => x[EFacetType.Range]).Returns(new RangeCategoryCountHelper(config));
            mockHelpers.Setup(x => x[EFacetType.Intersect]).Returns(new RangeCategoryCountHelper(config));

            mockHelpers.Setup(x => x[EFacetType.Discrete]).Returns(new DiscreteCategoryCountHelper());
            mockHelpers.Setup(x => x[EFacetType.GeoPolygon]).Returns(new DiscreteCategoryCountHelper());
        ...
        public virtual Mock<IIndex<EFacetType, ICategoryInfoService>> MockCategoryInfoServices()
        {
            ...
            services
                .Setup(x => x[EFacetType.Intersect])
                .Returns(
                    MockCategoryInfoService(
                        new FacetContent.CategoryInfo
                        {
            ...
            services
                .Setup(x => x[EFacetType.GeoPolygon])
                .Returns(
                    MockCategoryInfoService(
                        new FacetContent.CategoryInfo
                        {
        ...
        private static Mock<ICategoryInfoService> MockCategoryInfoService(FacetContent.CategoryInfo returnValue)
        {
            ...
            service.Setup(c => c.GetCategoryInfo(It.IsAny<FacetsConfig2>(), It.IsAny<string>(), It.IsAny<object>())).Returns(returnValue);
            service.Setup(c => c.SqlCompiler).Returns(new Mock<ICategoryInfoSqlCompiler>().Object);
            return service;
        }
        ...
        public virtual Mock<IIndex<EFacetType, ICategoryCountSqlCompiler>> MockCategoryCountSqlCompilers(string returnSql)
        {
            ...
            sqlCompilers
                .Setup(x => x[EFacetType.Range])
                .Returns(MockCategoryCountSqlCompiler<IRangeCategoryCountSqlCompiler>(returnSql).Object);
            sqlCompilers
                .Setup(x => x[EFacetType.Discrete])
                .Returns(MockCategoryCountSqlCompiler<IDiscreteCategoryCountSqlCompiler>(returnSql).Object);
            sqlCompilers
                .Setup(x => x[EFacetType.Intersect])
                .Returns(MockCategoryCountSqlCompiler<IIntersectCategoryCountSqlCompiler>(returnSql).Object);
            sqlCompilers
                .Setup(x => x[EFacetType.GeoPolygon])
                .Returns(MockCategoryCountSqlCompiler<IGeoPolygonCategoryCountSqlCompiler>(returnSql).Object);
            return sqlCompilers;
        }
        ...
        public virtual Mock<IPickFilterCompilerLocator> MockPickCompilerLocator(string returnValue = "")
        {
            ...
            mockLocator.Setup(x => x.Locate(It.IsAny<EFacetType>())).Returns(mockPickCompiler.Object);
        ...
        public virtual Mock<IPickFilterCompilerLocator> MockConcretePickCompilerLocator()
        {
            ...
            {
                mockLocator.Setup(x => x.Locate(item.Key)).Returns(Activator.CreateInstance(item.Value) as IPickFilterCompiler);
            }
            return mockLocator;
        }
        ...
        public virtual Mock<IPathFinder> MockFacetsGraph(List<Route> returnRoutes)
        {
            ...
            mockFacetsGraph.Setup(x => x.Find(It.IsAny<string>(), It.IsAny<List<string>>(), It.IsAny<bool>())).Returns(returnRoutes);
        ...
        public TableRelation FakeTableRelation(string sourceName, string targetName)
        {
            return new TableRelation
            {
                ...
                {
                    TableId = sourceName.GetHashCode(),
                    TableOrUdfName = sourceName,
                    IsUdf = sourceName.Contains('('),
                    PrimaryKeyName = "X",
                },
                TargetTable = new Table
                {
                    TableId = targetName.GetHashCode(),
                    TableOrUdfName = targetName,
                    IsUdf = targetName.Contains('('),
                    PrimaryKeyName = "X",
                },
                SourceColumnName = sourceName + "_id",
        ...
        public virtual Mock<IResultSqlCompilerLocator> MockResultSqlCompilerLocator(string returnSql)
        {
            ...
            mockResultSqlCompilerLocator.Setup(z => z.Locate(It.IsAny<string>())).Returns(mockResultSqlCompiler.Object);
