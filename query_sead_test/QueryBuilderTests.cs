        ...
        public void CanBuildCategoryCountQuerySetupForSingleDiscreteFacetWithoutPicks()
        {
            ...
            foreach (var facetCode in singleExpectedRoutes.Keys)
            {
                ...
                using (var scope = container.BeginLifetimeScope())
                {
                    ...
                    Assert.AreEqual(2, querySetup.reduced_routes.Count);
                    Assert.AreEqual(3, querySetup.reduced_routes[0].Items.Count);
                    Assert.AreEqual(1, querySetup.reduced_routes[1].Items.Count);
