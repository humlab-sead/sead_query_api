...
public class FacetsConfig2
{
    ...
    { // Current client facet configurations
        get { return facetConfigs; }
        set { facetConfigs = value?.OrderBy(z => z.Position).ToList(); }
    }
    ...
    public bool IsPriorTo(FacetConfig2 facetConfig, List<string> facetCodes, Facet targetFacet)
    {
        ...
        {
            // Ignore facets that do not have constraints or picks
            // TODO Should we include facetConfig in final query to get an implicit filter by inner join?
            return false;
        }
    ...
    public List<FacetConfig2> GetFacetConfigsAffectedBy(Facet facet, List<string> facetCodes)
    {
        ...
        return GetConfigs(facetCodes).Where(c => IsPriorTo(c, facetCodes, facet)).ToList();
    }

    public FacetConfig2 GetAt(int index)
    {
        if (index < 0 || index >= FacetConfigs.Count)
            throw new ArgumentOutOfRangeException(nameof(index), "Index is out of range for FacetConfigs.");

        return FacetConfigs[index];
    }

    public List<FacetConfig2> Take(int index)
    {
        if (index < 0 || index >= FacetConfigs.Count)
            throw new ArgumentOutOfRangeException(nameof(index), "Index is out of range for FacetConfigs.");
        return [.. FacetConfigs.Take(index + 1)];
    }

    public int IndexOf(FacetConfig2 config)
    {
        if (config == null)
            throw new ArgumentNullException(nameof(config), "FacetConfig2 cannot be null.");

        return FacetConfigs.IndexOf(config);
    }
    ...
    public List<Facet> GetInvolvedFacets(Facet facet, List<string> facetCodes = null) =>
        GetInvolvedConfigs(facet, facetCodes).Facets().AddUnion(facet).ToList();

    public List<string> GetInvolvedTables(Facet facet, List<string> facetCodes = null, List<string> extraTables = null) =>
        GetInvolvedFacets(facet, facetCodes).TableNames().NullableUnion(extraTables).Distinct().ToList();
    ...
    public bool HasDomainCode() => !string.IsNullOrEmpty(DomainCode);

    public FacetConfig2 CreateDomainConfig() => HasDomainCode() ? FacetConfigFactory.Create(DomainFacet, 0) : null;
}
