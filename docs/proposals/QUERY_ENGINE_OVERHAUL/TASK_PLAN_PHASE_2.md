...
| Area | Status | Notes |
|---|---|---|
| Discrete target inventory and exception list | In progress | Inventory work has now produced one grouped same-table target-only live promotion candidate in `result_facet:result_facet` plus one focused-live same-table validation in `result_datasets:result_datasets`; routed target-only widening still accounts for most of the validated surface and the remaining explicit blockers. |
| Target-side discrete widening | In progress | Same-table target-only discrete requests use an explicit unfiltered anchor query, `result_facet:result_facet` validates that path in the grouped visible matrix, and `result_datasets:result_datasets` now validates the same path in focused live coverage outside that grouped subset; routed zero-predicate discrete requests still use legacy-style outer-category overlay on top of composed counts. |
...
| Focused live promotion | In progress | `result_facet:result_facet`, `genus:genus`, `sites:sites`, `sample_groups:sample_groups`, `data_types:data_types`, `modification_types:modification_types`, `location_types:location_types`, `rdb_codes:rdb_codes`, `rdb_systems:rdb_systems`, `relative_age_name:relative_age_name`, `record_types:record_types`, `dataset_provider:dataset_provider`, `dataset_methods:dataset_methods`, `activeseason:activeseason`, `ecocode:ecocode`, `ecocode_system:ecocode_system`, `species_author:species_author`, `tbl_biblio_sample_groups:tbl_biblio_sample_groups`, `tbl_biblio_sites:tbl_biblio_sites`, and `tbl_biblio_modern:tbl_biblio_modern` now pass focused composed-path validation and have been promoted into `SupportedComposedVisibleAndDiscreteLiveUris`; `result_datasets:result_datasets` now also passes focused live parity on the same-table target-only path, but stays outside that grouped visible subset; `abundance_classification:abundance_classification`, `abundance_elements:abundance_elements`, `construction_purpose:construction_purpose`, `constructions:constructions`, `country:country`, `region:region`, `feature_type:feature_type`, `family:family`, `sample_group_sampling_contexts:sample_group_sampling_contexts`, and `species:species` were probed and still overcount on the target-only composed path, so they are not clean promotion candidates yet. |
| Documentation and phase exit | In progress | `PARITY_INVENTORY.md` and this plan now reflect the promoted target-only batch through `location_types:location_types`; update `docs/DESIGN.md` only when the runtime boundary changes rather than when the validated matrix widens within the same contract. |
...
- [ ] Select and probe the next remaining target-only discrete live slice after the focused `result_datasets:result_datasets` validation, then continue inventorying the remaining lower-confidence candidates once that result is known.
