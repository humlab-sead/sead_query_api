...
| Area | Status | Notes |
|---|---|---|
| Discrete target inventory and exception list | In progress | Inventory work has now produced one clean same-table target-only live promotion candidate in `result_facet:result_facet`; routed target-only widening still accounts for most of the validated surface and the remaining explicit blockers. |
| Target-side discrete widening | In progress | Same-table target-only discrete requests use an explicit unfiltered anchor query, and `result_facet:result_facet` now validates that path in the grouped live matrix; routed zero-predicate discrete requests still use legacy-style outer-category overlay on top of composed counts. |
...
- [ ] Select and probe the next remaining target-only discrete live slice after the `construction_purpose:construction_purpose` overcount result, then continue inventorying the remaining lower-confidence candidates once that result is known.
