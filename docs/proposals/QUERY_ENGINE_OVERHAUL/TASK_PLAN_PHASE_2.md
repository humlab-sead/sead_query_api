...
- [ ] Select and probe the next routed target-only discrete live slice after the `rdb_codes:rdb_codes` promotion batch, then continue inventorying the remaining target-only targets once that result is known.
