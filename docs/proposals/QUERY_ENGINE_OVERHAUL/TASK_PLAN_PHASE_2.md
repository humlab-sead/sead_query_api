...
- [ ] Select and probe the next remaining target-only discrete live slice after the `location_types:location_types` promotion batch, then continue inventorying the remaining lower-confidence candidates once that result is known.
