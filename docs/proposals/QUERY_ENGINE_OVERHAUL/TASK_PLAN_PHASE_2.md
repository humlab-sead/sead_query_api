...
| Focused live promotion | In progress | `genus:genus`, `sites:sites`, `sample_groups:sample_groups`, `data_types:data_types`, `rdb_systems:rdb_systems`, `relative_age_name:relative_age_name`, `record_types:record_types`, `dataset_provider:dataset_provider`, `dataset_methods:dataset_methods`, `activeseason:activeseason`, `ecocode:ecocode`, `ecocode_system:ecocode_system`, `species_author:species_author`, `tbl_biblio_sample_groups:tbl_biblio_sample_groups`, and `tbl_biblio_sites:tbl_biblio_sites` now pass focused composed-path validation and have been promoted into `SupportedComposedVisibleAndDiscreteLiveUris`; `abundance_classification:abundance_classification`, `country:country`, `region:region`, `feature_type:feature_type`, `family:family`, and `species:species` were probed and still overcount on the target-only composed path, so they are not clean promotion candidates yet. |
| Documentation and phase exit | In progress | `PARITY_INVENTORY.md` and this plan now reflect the promoted routed target-only batch through `tbl_biblio_sites:tbl_biblio_sites`; update `docs/DESIGN.md` only when the runtime boundary changes rather than when the validated matrix widens within the same contract. |
...
- [x] Review `PARITY_INVENTORY.md` after the `genus:genus`, `sites:sites`, `sample_groups:sample_groups`, `data_types:data_types`, `rdb_systems:rdb_systems`, `relative_age_name:relative_age_name`, `record_types:record_types`, `dataset_provider:dataset_provider`, `dataset_methods:dataset_methods`, `activeseason:activeseason`, `ecocode:ecocode`, `ecocode_system:ecocode_system`, `species_author:species_author`, `tbl_biblio_sample_groups:tbl_biblio_sample_groups`, and `tbl_biblio_sites:tbl_biblio_sites` promotion batches to confirm support and exception rows remain current.
...
- [ ] Select and probe the next routed target-only discrete live slice after the `tbl_biblio_sites:tbl_biblio_sites` promotion batch, then continue inventorying the remaining target-only targets once that result is known.
