...
- [ ] Select and probe the next routed target-only discrete live slice after the `tbl_biblio_modern:tbl_biblio_modern` promotion batch, then continue inventorying the remaining target-only targets once that result is known.
