...
- [ ] Select and probe the next remaining target-only discrete live slice after the `result_facet:result_facet` promotion batch, then continue inventorying the remaining lower-confidence candidates once that result is known.
