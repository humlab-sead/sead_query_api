...
| Area | Status | Notes |
|---|---|---|
| Discrete target inventory and exception list | In progress | Inventory work has now produced one grouped same-table target-only live promotion candidate in `result_facet:result_facet` plus focused-live same-table validations in `result_datasets:result_datasets` and `map_result:map_result`; the remaining nearby same-table residue is the non-applicable helper facet `sites_helper`, so routed target-only widening still accounts for most remaining parity risk. |
| Target-side discrete widening | In progress | Same-table target-only discrete requests use an explicit unfiltered anchor query, `result_facet:result_facet` validates that path in the grouped visible matrix, and `result_datasets:result_datasets` plus `map_result:map_result` now validate the same path in focused live coverage outside that grouped subset; routed zero-predicate discrete requests still use legacy-style outer-category overlay on top of composed counts. |
...
- [ ] Decide whether any follow-up probe beyond `result_datasets:result_datasets` and `map_result:map_result` is still in-scope for Phase 2, given that the remaining nearby same-table residue is the non-applicable helper `sites_helper` and the open risk is now concentrated in routed overcount blockers.
