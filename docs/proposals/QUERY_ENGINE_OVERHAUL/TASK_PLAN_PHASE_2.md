...
| Documentation and phase exit | In progress | `PARITY_INVENTORY.md` and this plan now reflect the promoted routed target-only batch through `tbl_biblio_modern:tbl_biblio_modern`; update `docs/DESIGN.md` only when the runtime boundary changes rather than when the validated matrix widens within the same contract. |
...
- [ ] Select and probe the next routed target-only discrete live slice after the `abundance_elements:abundance_elements` overcount result, then continue inventorying the remaining target-only targets once that result is known.
