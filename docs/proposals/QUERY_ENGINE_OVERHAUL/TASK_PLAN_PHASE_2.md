...
| Focused live promotion | In progress | `genus:genus`, `sites:sites`, `sample_groups:sample_groups`, `data_types:data_types`, `rdb_systems:rdb_systems`, `relative_age_name:relative_age_name`, `record_types:record_types`, `dataset_provider:dataset_provider`, `dataset_methods:dataset_methods`, `activeseason:activeseason`, and `ecocode:ecocode` now pass focused composed-path validation and have been promoted into `SupportedComposedVisibleAndDiscreteLiveUris`; `abundance_classification:abundance_classification`, `region:region`, `feature_type:feature_type`, and `family:family` were probed and still overcount on the target-only composed path, so they are not clean promotion candidates yet. |
| Documentation and phase exit | In progress | `PARITY_INVENTORY.md` and this plan now reflect the promoted routed target-only batch through `ecocode:ecocode`; update `docs/DESIGN.md` only when the runtime boundary changes rather than when the validated matrix widens within the same contract. |
...
- [ ] Select and probe the next routed target-only discrete live slice after `ecocode:ecocode`, then continue inventorying the remaining target-only targets once that result is known.
