...
| Focused live promotion | In progress | `genus:genus`, `sites:sites`, and `sample_groups:sample_groups` now pass focused composed-path validation and have been promoted into `SupportedComposedVisibleAndDiscreteLiveUris`; the next routed zero-predicate probe should be selected from the remaining target-only discrete inventory. |
| Documentation and phase exit | In progress | `PARITY_INVENTORY.md`, `docs/DESIGN.md`, and this plan were updated for the first target-only discrete batch so the current boundary is explicit. |
...
- [x] Review `PARITY_INVENTORY.md` after the `genus:genus`, `sites:sites`, and `sample_groups:sample_groups` promotion batches to confirm support and exception rows remain current.
...
- [ ] Select the next routed zero-predicate discrete live slice from the remaining target-only inventory now that `genus:genus`, `sites:sites`, and `sample_groups:sample_groups` validate on the same contract.
