...
- [ ] Select and probe the next routed target-only discrete live slice after the `constructions:constructions` overcount result, then continue inventorying the remaining target-only targets once that result is known.
