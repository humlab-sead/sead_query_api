...
- [ ] Select and probe the next routed target-only discrete live slice after the `modification_types:modification_types` promotion batch, then continue inventorying the remaining target-only targets once that result is known.
