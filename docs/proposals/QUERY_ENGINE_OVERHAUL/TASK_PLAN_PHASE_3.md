...
| Area | Status | Notes |
|---|---|---|
| Non-discrete inventory and exception list | In progress | `PARITY_INVENTORY.md` now shows supported range widening, supported intersect widening, and unsupported GIS polygon fallback. The active catalog checks for Phase 3 found no additional real range or intersect candidates beyond the already-supported set and helper-only residue, while the routed discrete overcount set from Phase 2 (`abundance_classification`, `abundance_elements`, `construction_purpose`, `constructions`, `country`, `region`, `feature_type`, `family`, `sample_group_sampling_contexts`, and `species`) remains on the discrete exception track unless a shared non-discrete contract gap is proven. |
