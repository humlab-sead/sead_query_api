...
| Intersect facet support | In progress | `analysis_entity_ages:analysis_entity_ages` now runs on the composed path with focused unit coverage, focused live parity, and grouped live coverage through `SupportedComposedIntersectLiveUris`. The remaining intersect surface still needs inventory and explicit widening or exception decisions. |
