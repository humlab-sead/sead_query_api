...
- The next validated post-range boundary shows that discrete targets can also use the composed path when the join key stays routable on the root target table (`abundance_classification` and `activeseason`), when sibling joined target tables contribute enforced target-side clauses (`tbl_biblio_sample_groups`, `tbl_biblio_sites`), when the same bibliography target family routes through a different aggregate helper (`tbl_biblio_modern`), and when a multi-table location target family stays within the existing routed-target contract (`region`).
