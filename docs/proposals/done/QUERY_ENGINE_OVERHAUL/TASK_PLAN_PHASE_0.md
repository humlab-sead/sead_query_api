# Task Plan: Phase 0 Vertical Slice

## Status

- Document type: working task plan
...
- [x] `region:country@1,2,5/region`
- [x] `activeseason:country@1,2,5/activeseason`
- [x] `sample_groups:country@1,2,5/sample_groups`
- [x] `data_types:country@1,2,5/data_types`
- [x] `rdb_systems:country@1,2,5/rdb_systems`
...
- The same post-range discrete widening also covers `region:country@1,2,5/region`, so a multi-table location target family still routes through the composed path without additional runtime changes.
- The same post-range discrete widening also covers `activeseason:country@1,2,5/activeseason`, so plain single-table discrete targets on the same predicate side still fit the current composed contract without additional runtime changes.
- The same post-range discrete widening also covers `sample_groups:country@1,2,5/sample_groups`, so the existing routed visible-target contract also holds when the target facet is a previously validated baseline visible target exercised under the `country`-predicate side.
- The same post-range discrete widening also covers `data_types:country@1,2,5/data_types`, so routed visible targets that anchor through datasets still fit the current composed contract without additional runtime changes.
- The same post-range discrete widening also covers `rdb_systems:country@1,2,5/rdb_systems`, so location-routed visible targets whose category names depend on joined location tables still fit the current composed contract without additional runtime changes.
...
- Current adjacent validated discrete multi-table location scenario: `region:country@1,2,5/region` through `IFacetContentService`.
- Current adjacent validated discrete single-table scenario: `activeseason:country@1,2,5/activeseason` through `IFacetContentService`.
- Current adjacent validated discrete routed visible-target scenario: `sample_groups:country@1,2,5/sample_groups` through `IFacetContentService`.
- Current adjacent validated discrete dataset-routed visible-target scenario: `data_types:country@1,2,5/data_types` through `IFacetContentService`.
- Current adjacent validated discrete location-routed visible-target scenario: `rdb_systems:country@1,2,5/rdb_systems` through `IFacetContentService`.
...
- The next validated post-range boundary shows that discrete targets can also use the composed path when the join key stays routable on the root target table (`abundance_classification` and `activeseason`), when sibling joined target tables contribute enforced target-side clauses (`tbl_biblio_sample_groups`, `tbl_biblio_sites`), when the same bibliography target family routes through a different aggregate helper (`tbl_biblio_modern`), when a multi-table location target family stays within the existing routed-target contract (`region`), and when baseline visible targets are exercised as predicate-side widening discriminators through sample-group routing (`sample_groups`), dataset routing (`data_types`), or location routing with joined location-name expressions (`rdb_systems`).
...
1. Pick the next adjacent discrete target on the same `country@1,2,5` predicate side and validate whether it fits one of the now-supported post-range patterns or exposes a genuinely new join boundary.
2. If the next target needs a different target-root, join-column, or predicate-source contract, document that difference explicitly before widening further.
