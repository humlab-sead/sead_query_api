...
select * from facet.facet where facet_key = 'species'
...
create table facet.result_field (
	...
	field_type_id varchar(20) not null,
...
select trim('    **** ')
drop table facet.result_aggregate_field;
drop table facet.result_aggregate;
drop table facet.result_type;

create table facet.result_aggregate (
	aggregate_id int not null,
	aggregation_key varchar(40) not null,
	display_text varchar(80) not null,
	...
	CONSTRAINT result_aggregate_pk PRIMARY KEY (aggregate_id)
);

insert into facet.result_aggregate (aggregate_id, aggregation_key, display_text)
	values (1, 'site_level','Site level');
insert into facet.result_aggregate (aggregate_id, aggregation_key, display_text)
	values (2, 'aggregate_all','Aggregate all');
insert into facet.result_aggregate (aggregate_id, aggregation_key, display_text)
	values (3, 'sample_group_level','Sample group level');
...
--alter table facet.result_type add column is_result_value boolean not null default(true);
--update facet.result_type set is_result_value = false where result_type_id = 5;

create table facet.result_aggregate_field (
	aggregate_field_id serial not null,
	aggregate_id int not null,
	result_field_id int not null,
	field_type_id varchar(40) not null,
	CONSTRAINT result_aggregate_field_pk PRIMARY KEY (aggregate_field_id),
	CONSTRAINT result_aggregate_fieldfk1 FOREIGN KEY (aggregate_id)
	  REFERENCES facet.result_aggregate (aggregate_id) 
	  ON UPDATE NO ACTION ON DELETE NO ACTION,
	CONSTRAINT result_aggregate_fieldfk2 FOREIGN KEY (result_field_id)
	  REFERENCES facet.result_field (result_field_id) 
	  ON UPDATE NO ACTION ON DELETE NO ACTION,
	CONSTRAINT result_aggregate_fieldfk3 FOREIGN KEY (field_type_id)
